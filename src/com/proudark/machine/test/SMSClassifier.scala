...
object SMSClassifier {
   def main(args: Array[String]) {
     ...
     val VECTOR_SIZE = 100
     val parsedRDD = sc.textFile("/root/seed.txt").map(_.split(",")).map(eachRow => {
     ...
     val layers = Array[Int](VECTOR_SIZE,6,7,8)
     ...
     predictionResultDF.select("label","prediction", "predictedLabel").write.mode(SaveMode.Overwrite).json("/root/SeedResult")
   }
}